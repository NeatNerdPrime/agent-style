# agent-style bench — v0.2.0 (runner: Gemini CLI (pro))

- Runner: Gemini CLI (pro)
- Generations per condition: 2
- Date: 2026-04-21T07:55:19Z
- agent-style version: 0.2.0

## Per-task per-rule delta (mechanical only; structural and semantic excluded)

| Task | Baseline total | Treatment total | Delta | Dominant rule |
|---|---|---|---|---|
| pr-01-redis-cache | 1 | 0 | -1 | — |
| pr-03-auth-middleware | 1 | 0 | -1 | RULE-I |
| design-02-rate-limiter | 10 | 0 | -10 | RULE-12 |
| commit-01-fix-timezone | 0 | 0 | 0 | — |
